...
package com.andee_ai_openbot.original;
...
import com.andee_ai_openbot.OpenBotApplication;
import com.andee_ai_openbot.env.AudioPlayer;
import com.andee_ai_openbot.env.BotToControllerEventBus;
import com.andee_ai_openbot.env.ControllerToBotEventBus;
import com.andee_ai_openbot.env.GameController;
import com.andee_ai_openbot.env.ImageUtils;
import com.andee_ai_openbot.env.Logger;
import com.andee_ai_openbot.env.PhoneController;
import com.andee_ai_openbot.env.SharedPreferencesManager;
import com.andee_ai_openbot.logging.LogDataUtils;
import com.andee_ai_openbot.logging.SensorService;
import com.andee_ai_openbot.server.ServerCommunication;
import com.andee_ai_openbot.server.ServerListener;
import com.andee_ai_openbot.utils.ConnectionUtils;
import com.andee_ai_openbot.utils.Constants;
import com.andee_ai_openbot.utils.Enums;
import com.andee_ai_openbot.utils.FileUtils;
import com.andee_ai_openbot.utils.FormatUtils;
import com.andee_ai_openbot.utils.PermissionUtils;
import com.andee_ai_openbot.vehicle.Vehicle;
...
import com.andee_ai_openbot.R;

import com.andee_ai_openbot.tflite.Model;
import com.andee_ai_openbot.tflite.Network.Device;
import com.andee_ai_openbot.vehicle.Control;
...
public abstract class CameraActivity extends AppCompatActivity
  ...
  protected Enums.ControlMode controlMode = Enums.ControlMode.GAMEPAD;
  protected Enums.SpeedMode speedMode = Enums.SpeedMode.NORMAL;
  ...
  private void setInitialValues() {
    ...
    if (Enums.ControlMode.getByID(preferencesManager.getControlMode()) != null)
      controlModeSpinner.setSelection(
          Objects.requireNonNull(Enums.ControlMode.getByID(preferencesManager.getControlMode()))
              .ordinal());
    if (Enums.DriveMode.getByID(preferencesManager.getDriveMode()) != null)
      driveModeSpinner.setSelection(
          Objects.requireNonNull(Enums.DriveMode.getByID(preferencesManager.getDriveMode())).ordinal());
    if (Enums.SpeedMode.getByID(preferencesManager.getSpeedMode()) != null)
      speedModeSpinner.setSelection(
          Objects.requireNonNull(Enums.SpeedMode.getByID(preferencesManager.getSpeedMode())).ordinal());
  ...
  private void setLogMode(Enums.LogMode logMode) {
  ...
  private void connectPhoneController() {
    ...
    Enums.DriveMode oldDriveMode = driveMode;
  ...
  private void disconnectPhoneController() {
    phoneController.disconnect();
    setDriveMode(Enums.DriveMode.values()[preferencesManager.getDriveMode()]);
  ...
  public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
    ...
    if (parent == baudRateSpinner) {
      ...
      setLogMode(Enums.LogMode.valueOf(selected.toUpperCase()));
    } else if (parent == controlModeSpinner) {
      setControlMode(Enums.ControlMode.valueOf(selected.toUpperCase()));
    } else if (parent == driveModeSpinner) {
      setDriveMode(Enums.DriveMode.valueOf(parent.getItemAtPosition(pos).toString().toUpperCase()));
    } else if (parent == speedModeSpinner) {
      setSpeedMode(Enums.SpeedMode.valueOf(selected.toUpperCase()));
    }
  }
  ...
  */
  ...
  protected class ControllerHandler {
    ...
    protected void handleDriveMode() {
      ...
      // Set next drive mode
      setDriveMode(Enums.DriveMode.values()[(driveMode.getValue() + 1) % Enums.DriveMode.values().length]);
    ...
    protected void handleSpeedUp() {
      ...
      switch (speedMode) {
        case SLOW:
          setSpeedMode(Enums.SpeedMode.NORMAL);
          break;
        case NORMAL:
          setSpeedMode(Enums.SpeedMode.FAST);
          break;
        default:
          break;
      }
    ...
    protected void handleSpeedDown() {
      ...
      switch (speedMode) {
        case FAST:
          ...
          break;
        case NORMAL:
          setSpeedMode(Enums.SpeedMode.SLOW);
          break;
        default:
          break;
      }
